...
| Timer | 从午夜开始的秒数 | ✅ 通过 | - |
| DateAdd | 日期加减 | ✅ 通过 | - |
| DateDiff | 日期差值 | ⚠️ 部分通过 | 简化实现 |
| DatePart | 日期部分 | ✅ 通过 | - |
| WeekdayName | 星期名称 | ✅ 通过 | - |
| MonthName | 月份名称 | ✅ 通过 | - |
| DateValue | 日期转换 | ⚠️ 部分通过 | 简化实现 |
| TimeValue | 时间转换 | ⚠️ 部分通过 | 简化实现 |
| DateSerial | 日期序列化 | ⚠️ 部分通过 | 简化实现 |
| TimeSerial | 时间序列化 | ⚠️ 部分通过 | 简化实现 |
...
- `DateDiff` 实现较为简化，某些边界情况可能不准确
- `DateValue`, `TimeValue` 使用简化解析

### 2. 类型判断函数

- `IsDate` 使用简化实现，可能误判某些格式
...
| 2024-XX-XX | 初始测试文件创建 |
| 2024-XX-XX | 添加数学函数测试 |
| 2024-XX-XX | 添加字符串函数测试 |
| 2024-XX-XX | 添加日期时间函数测试 |
| 2024-XX-XX | 添加数组和判断函数测试 |
